...
import { GetAvailablePropertiesArgs, Property, PropertyImage, useGetAvailablePropertiesQuery } from '@/rtk/api/showproperty';
...
const healthHomes = data?.properties ?? [];
  ...
  const getHealthHomesByCity = (city: string) =>
    filteredHomes.filter((p) => (p.city || "").trim().toLowerCase() === city.trim().toLowerCase());
    ...
    return (
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4">
                    ...
                    >
                      <div className="relative h-40 overflow-hidden">
                        ...
                        <div className="absolute top-2 left-2 bg-primary text-primary-foreground rounded-lg px-2 py-1">
                          <span className="text-xs font-semibold">{home.propertyType.toUpperCase()}</span>
                        </div>
                      </div>
         ...
         {/* ✅ Desktop Carousel (3 cards per frame + arrows) */}
        <div className="hidden md:block">
          <Carousel
            opts={{ align: "start", loop: displayedHomes.length > 3 }}
            className="w-full relative"
          >
            <CarouselContent className="-ml-4">
              {displayedHomes.map((home) => (
                <CarouselItem key={home.id} className="pl-4 basis-1/3">
                  ...
                  >
                    <div className="relative h-32 md:h-40 overflow-hidden">
                      <img
                        src={getCardImage(home as unknown as Property)}
                        alt={home.name}
                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                      />
                      ...
                      <div className="mb-3">
                        ...
                        <div className="flex items-center gap-1 text-muted-foreground mb-2">
                          ...
                          <span className="text-xs line-clamp-1">
                            {getLocation(home as unknown as Property)}
                          </span>
                        </div>
